import { firestoreAction } from 'vuexfire'
import { dbOrderRef } from '../../firebase'

const state = {
    orders: []
}

const mutations = {
    // addOrder: (state, orders) => state.orders.push(orders)
}

const getters = {
    numberOfOrders: state => state.orders.length,
    getOrders: state => state.orders
}

const actions = {
    setOrdersRef: firestoreAction(context => {
        return context.bindFirestoreRef('orders', dbOrderRef.orderBy('createdAt') )
    }),
    addNewOrder: async(context, order) => {
        try{
        await dbOrderRef.add(order)
        }
        catch(error){
            alert('Sorry, there was a problem placing your oder. Please try again.')
        }
    },
    removeOrder: async(context, id) => {
        try{
            const order = await dbOrderRef.doc(id)
            order.delete()
        }
        catch(error)
        {
            alert(`Sorry, there was a problem removng the order, ${error}`)
        }

    }
}

export default{
    state,
    mutations, 
    getters,
    actions
}